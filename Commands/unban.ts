        ...
        if (!message.member.hasPermission("BAN_MEMBERS")) {
            ...
            if (message.content.length >= 2 && message.content.split(' ')[1].length == 18 && Number(message.content.split(' ')[1])) {
                message.guild.members.unban(message.content.split(' ')[1])
                    ...
                    .catch((error: any) => message.channel.send({
                        embed: CreateEmbed(
                            'fail', 'Error', '', 'Please use proper id for unbanning the user', [], '', ''
                        )
                    }));
            }
            else {

                message.channel.send({ embed: CreateEmbed('fail', "Couldn't Unban", '', 'Please provide the id as the first argument followed by the reason [Note the reason is optional]', [], '', '') })
            }
        }
    }
}